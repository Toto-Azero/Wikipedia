...
#
__version__ = '$Id: hotarticles.py 0100 2016-02-07 22:11:30 (CET) Toto Azéro $'
#
...
AS count_minor, COUNT(DISTINCT rc_user) as nb_users, SUM(rc_new_len - rc_old_len) as diff \
FROM recentchanges \
JOIN (SELECT page_id, page_title FROM categorylinks \
JOIN page ON page_id=cl_from AND page_namespace=0 \
...
if __name__ == '__main__':
	try:
		...
		#pywikibot.output([p for p in gen])
		for page in gen:
			try:
